...
export const Title = ({ text, link, hasMore }: { text: string; link?: string, hasMore?: boolean }) => {
    return (
        <div className="flex text-xl md:text-4xl px-5 gap-5 mt-10 flex-row items-center justify-center text-foreground"> {/* Using text-gray-100 for foreground */}
            ...
            <div className="flex w-full flex-row gap-10 md:gap-30 xl:gap-40 items-center">
                <div className="flex w-5/6 h-1 bg-callout"></div> {/* Using direct hex for bg-callout */}
                {hasMore && (
                    link ? (
                        <a href={link} className="flex flex-row text-xl items-center text-foreground hover:text-callout transition-colors duration-300">
